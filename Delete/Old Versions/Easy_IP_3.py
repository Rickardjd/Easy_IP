...
class DeviceType(IntEnum):
    """Device type identifiers from tag 0x00a6"""
    CAMERA = 0x91  # Standard camera
    RECORDER = 0x92  # Network video recorder
...
class DeviceInfo:
    ...
    channels: Optional[int] = None  # For recorders - number of channels
    capacity: Optional[int] = None  # For recorders - storage capacity indicator
    ...
    def to_csv_row(self) -> List[str]:
        ...
        return [
            ...
            str(self.channels) if self.channels else "",
    ...
    def __str__(self) -> str:
        ...
        # Add recorder-specific info
        if self.device_type == "recorder":
            if self.channels:
...
class iPROIPSetup:
    ...
    def _parse_response(self, data: bytes, addr: tuple) -> Optional[DeviceInfo]:
        ...
        try:
            ...
            # *** CRITICAL: 0xa6 = Device Type (Camera vs Recorder) ***
            device_type = "camera"  # Default to camera for backward compatibility
            device_type_code = None
            
            if 0xa6 in tlv_data and len(tlv_data[0xa6]) >= 1:
                device_type_code = tlv_data[0xa6][0]
                if device_type_code == DeviceType.CAMERA:
                    device_type = "camera"
                    logger.debug(f"Device type: CAMERA (0x{device_type_code:02x})")
                elif device_type_code == DeviceType.RECORDER:
                    device_type = "recorder"
                    logger.debug(f"Device type: RECORDER (0x{device_type_code:02x})")
                else:
                    device_type = f"unknown_0x{device_type_code:02x}"
                    logger.warning(f"Unknown device type: 0x{device_type_code:02x}")
            
            # 0x00/0x01 = Network mode
            ...
            if device_type == "recorder":
                # 0xc0 = Channel count (2 bytes)
                if 0xc0 in tlv_data and len(tlv_data[0xc0]) == 2:
                    channels = struct.unpack(">H", tlv_data[0xc0])[0]
                    logger.debug(f"Channels from TLV 0xc0: {channels}")
                
                # 0xc1 = Capacity (2 bytes)
                if 0xc1 in tlv_data and len(tlv_data[0xc1]) == 2:
                    capacity = struct.unpack(">H", tlv_data[0xc1])[0]
                    logger.debug(f"Capacity from TLV 0xc1: {capacity}")
